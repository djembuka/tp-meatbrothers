	...
	BX.Sale.OrderAjaxComponent = {
					...
					events: {
						onAfterPopupShow: BX.proxy(function(){
							...
							this.loadingScreen.popupContainer.appendChild(
								BX.create('IMG', {props: {src: this.templateFolder + "/images/preloader.png", width: '64', height: '64' }})
							);
		...
		editFadeRegionContent: function(node)
		{
			...
			if (selectedPersonType && selectedPersonType.NAME && BX.util.object_keys(this.result.PERSON_TYPE).length > 1)
				addedHtml += '<span>' + BX.util.htmlspecialchars(this.params.MESS_PERSON_TYPE) + ':</span> ' + selectedPersonType.NAME + '<br />';
			...
			if (locationProperty && locationString.length)
				addedHtml += '<span>' + BX.util.htmlspecialchars(locationProperty.NAME) + ':</span> ' + locationString + '<br />';

			if (zipProperty && zipValue.length)
				addedHtml += '<span>' + BX.util.htmlspecialchars(zipProperty.NAME) + ':</span> ' + zipValue;
				...
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						...
						label = BX.create('LABEL', {
							children: [
								BX.create('INPUT', {
									attrs: {checked: currentType.CHECKED == 'Y', className: 'with-gap'},
		...
		editFadePaySystemContent: function(node)
		{
			...
			if (this.isSelectedInnerPayment())
			{
				...
				addedHtml += '<span>' + this.result.INNER_PAY_SYSTEM.NAME + '</span><br />';
				addedHtml += '</div>';
			}
		...
		editTotalBlock: function()
		{
			...
			this.totalInfoBlockNode.appendChild(this.createTotalUnit($('#js_you_order').text(), '<a href="/personal/cart/" class="bx-soa-editstep">'+$('#js_edit').text()+'</a>'));
	...
	(function($) {
		...
		$( function() {
			...
			$( document ).scroll( function() {
				...
				var l = $bottomButton.offset().top + $bottomButton.height() - $( '#bx-soa-total .bx-soa-cart-total' ).offset().top;
