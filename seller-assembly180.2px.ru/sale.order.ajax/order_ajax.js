...
(function () {
  ...
  BX.Sale.OrderAjaxComponent = {
    ...
    sendRequest: function (action, actionData) {
      ...
      if (!(loaderTimer = this.startLoader())) return;
    ...
    getAllFormData: function () {
      ...
      for (i in prepared.data)
        if (prepared.data.hasOwnProperty(i) && i == '') delete prepared.data[i];
    ...
    refreshOrder: function (result) {
      if (result.error) {
        ...
        var animation =
          this.result.OK_MESSAGE && this.result.OK_MESSAGE.length
            ? 'bx-step-good'
            : 'bx-step-bad';
          ...
          events: {
            onAfterPopupShow: BX.proxy(function () {
              ...
              this.loadingScreen.popupContainer.appendChild(
                BX.create('IMG', {
                  props: {
                    src: this.templateFolder + '/images/preloader.png',
                    width: '64',
                    height: '64',
                  },
                })
    ...
    checkNotifications: function () {
      ...
      if (informer) {
        if (
          ...
          !this.result.LAST_ORDER_DATA.FAIL
        ) {
    ...
    showWarnings: function () {
      ...
      if (currentDelivery && currentDelivery.CALCULATE_ERRORS) {
        ...
        warningString =
          '<strong>' +
          BX.util.htmlspecialchars(this.params.MESS_DELIVERY_CALC_ERROR_TITLE) +
          '</strong>';
    ...
    showBlockWarning: function (node, warnings, hide) {
      ...
      if (errorNode) {
        if (BX.type.isString(warnings)) warnStr = warnings;
          ...
          for (i in warnings)
            if (warnings.hasOwnProperty(i)) warnStr += warnings[i] + '<br />';
        }
          ...
          BX.create('LI', {
            ...
            html:
              pagination.pageNumber == 1
                ? '<span>' +
                  BX.util.htmlspecialchars(this.params.MESS_NAV_BACK) +
                  '</span>'
                : '<a href=""><span>' +
                  BX.util.htmlspecialchars(this.params.MESS_NAV_BACK) +
                  '</span></a>',
          ...
          BX.create('LI', {
            ...
            html:
              pagination.pageNumber == pagination.pages.length
                ? '<span>' +
                  BX.util.htmlspecialchars(this.params.MESS_NAV_FORWARD) +
                  '</span>'
                : '<a href=""><span>' +
                  BX.util.htmlspecialchars(this.params.MESS_NAV_FORWARD) +
                  '</span></a>',
    ...
    bindEvents: function () {
      BX.bind(
        this.orderSaveBlockNode.querySelector('a'),
        'click',
      ...
      BX.bind(
        ...
        BX.throttle(
          function () {
            ...
            this.basketBlockScrollCheck();
            if (this.mapsReady) this.resizeMapContainers();
          },
          50,
          this
        )
    ...
    clickOrderSaveAction: function (event) {
      ...
      if (this.isValidForm()) this.sendRequest('saveOrderAjax');
          ...
          if (this.params.SKIP_USELESS_BLOCK == 'Y') {
            ...
            if (onePickUp || oneDelivery || onePaySystem) {
              if (
                ...
                titleNode = nextSection.querySelector(
                  '.bx-soa-section-title-container'
                );
                BX.bind(titleNode, 'click', BX.proxy(this.showByClick, this));
              }
        ...
        if (allSections[i].id == actionSection.id && allSections[i - 1]) {
          ...
          if (
            (prevSection.id == this.deliveryBlockNode.id &&
              ...
              this.result.DELIVERY[0].EXTRA_SERVICES.length === 0 &&
              !this.result.DELIVERY[0].CALCULATE_ERRORS) ||
            (prevSection.id == this.paySystemBlockNode.id &&
              this.result.PAY_SYSTEM.length == 1 &&
              this.result.PAY_FROM_ACCOUNT != 'Y') ||
    ...
    changeVisibleContent: function () {
      ...
      if (
        ...
        this.switchOrderSaveButtons(
          sections[sections.length - 1].getAttribute('data-visited') == 'true'
        );
    },
    ...
    editOrder: function () {
      ...
      var sections = this.orderBlockNode.querySelectorAll(
        ...
        i;
      for (i in sections)
        if (sections.hasOwnProperty(i)) this.editSection(sections[i]);

      this.editTotalBlock();
      this.totalBlockFixFont();

      if (!this.result.SHOW_AUTH) this.changeVisibleContent();
    ...
    editSocialContent: function (authContent) {
      ...
      var nodes = [],
        socServiceHiddenNode = BX('bx-soa-soc-auth-services').querySelector(
          '.bx-authform-social'
        );
        ...
        nodes.push(
          BX.create('DIV', {
            ...
            children: [
              BX.create('H3', {
                props: { className: 'bx-title' },
                text: BX.message('SOA_DO_SOC_SERV'),
              }),
        ...
        BX.create('DIV', {
          ...
          children: [
            BX.create('h2', {
              ...
              text: BX.hasClass(insertContainer, 'reg')
    ...
    editActiveBasketBlock: function (activeNodeMode) {
      ...
      if (this.initialized.basket) {
        ...
        } else BX.cleanNode(basketContent);
      ...
      return BX.create('DIV', {
        ...
        children: active
          ? [
              BX.create('LABEL', {
                attr: { for: 'coupon' },
                text: this.params.MESS_USE_COUPON + ':',
              }),
            ]
    ...
    removeCoupon: function (coupon) {
      ...
      for (i in couponNodes)
        if (couponNodes.hasOwnProperty(i)) BX.remove(couponNodes[i]);
    },
    ...
    editActiveRegionBlock: function (activeNodeMode) {
      ...
      if (this.initialized.region) {
        ...
        this.getPersonPropsControl(regionNode);
    ...
    editFadeRegionContent: function (node) {
      ...
      if (
        ...
        BX.util.object_keys(this.result.PERSON_TYPE).length > 1
      )
        addedHtml +=
          '<span>' +
          BX.util.htmlspecialchars(this.params.MESS_PERSON_TYPE) +
          ':</span> ' +
          selectedPersonType.NAME +
          '<br />';
    ...
    getPersonPropsControl: function (node) {
      ...
      while ((group = groupIterator())) {
        if (group.getId() === '1' || group.getId() === '3') continue;
      ...
      node.appendChild(propsItemsContainer);
    },

    getPersonTypeControl: function (node) {
      if (!this.result.PERSON_TYPE) return;

      var personTypesCount = BX.util.object_keys(
          this.result.PERSON_TYPE
        ).length,
        currentType,
        oldPersonTypeId,
      ...
      if (profilesLength) {
        if (this.params.ALLOW_USER_PROFILES == 'Y') {
          ...
          if (profilesLength > 1 || this.params.ALLOW_NEW_PROFILE == 'Y') {
            label = BX.create('LABEL', {
              props: { className: 'bx-soa-custom-label' },
              text: this.params.MESS_SELECT_PROFILE,
            });
    ...
    createPaySystemItem: function (item) {
      ...
      itemNode = BX.create('DIV', {
        props: { className: 'bx-soa-pp-company col-xs-6' },
      ...
      if (currentPaySystem) {
        ...
        if (this.params.SHOW_PAY_SYSTEM_INFO_NAME == 'Y')
          subTitle = BX.create('DIV', {
            ...
            html: currentPaySystem.PSA_NAME,
          });
    ...
    getInnerPaySystem: function () {
      ...
      htmlString =
        BX.util.htmlspecialchars(this.params.MESS_INNER_PS_BALANCE) +
        ' <b class="wsnw">' +
        this.result.CURRENT_BUDGET_FORMATED +
        '</b><br />' +
    ...
    editFadePaySystemContent: function (node) {
      ...
      if (this.isSelectedInnerPayment()) {
        ...
        addedHtml +=
          '<span>' + this.result.INNER_PAY_SYSTEM.NAME + '</span><br />';
        addedHtml += '</div>';
      }
      ...
      if (selectedPaySystem && selectedPaySystem.PSA_NAME) {
        ...
        addedHtml += '<div class="bx-soa-pp-company-selected">';
        addedHtml += '<img src="' + imgSrc + '" style="height:18px;" alt="">';
        addedHtml += '<span>' + selectedPaySystem.PSA_NAME + '</span>';
        addedHtml += '</div>';
      }
    ...
    editFadeDeliveryContent: function (node) {
      ...
      if (selectedDelivery && selectedDelivery.NAME) {
        ...
        node.appendChild(
          BX.create('STRONG', { text: BX.message('SOA_DELIVERY_SELECT_ERROR') })
        );
    ...
    editActivePickUpBlock: function (activeNodeMode) {
      ...
      if (this.initialized.pickup) {
        ...
        pickUpContentRow = BX.create('DIV', {
    ...
    initMaps: function () {
      ...
      if (activeStores && activeStores.length) {
        ...
        this.pickUpMap.events.add(
          'click',
          BX.proxy(function () {
            if (this.pickUpMap.balloon.isOpen()) this.pickUpMap.balloon.close();
          }, this)
        );
        ...
        if (this.params.SHOW_NEAREST_PICKUP == 'Y') {
          ...
          provider =
            this.options.pickUpMap.secureGeoLocation &&
            BX.browser.IsChrome() &&
            !this.isHttps
              ? 'yandex'
              : 'auto';
        ...
        for (i = 0; i < activeStores.length; i++) {
          ...
          this.pickUpPointsJSON.push({
            ...
            geometry: {
              type: 'Point',
              coordinates: [activeStores[i].GPS_N, activeStores[i].GPS_S],
            },
              ...
              balloonContentLayout: ymaps.templateLayoutFactory.createClass(
                  ...
                  '<br /><a class="btn btn-sm btn-default" data-store="{{ properties.id }}">{{ properties.text }}</a>',
                {
                  ...
                  selectStoreByClick: function (e) {
                    ...
                    if (
                      BX.Sale.OrderAjaxComponent.pickUpMap.container.isFullscreen()
                    )
      ...
      if (this.params.SHOW_MAP_IN_PROPS == 'Y' && BX('propsMap')) {
        ...
        this.propsMap.events.add(
          ...
          BX.proxy(function (e) {
            ...
            } else this.propsMap.geoObjects.get(0).geometry.setCoordinates([coordinates[0], coordinates[1]]);
          }, this)
        );
      }
    },
    ...
    resizeMapContainers: function () {
      ...
      if (resizeBy && (pickUpMapContainer || propertyMapContainer)) {
        ...
        if (pickUpMapContainer) pickUpMapContainer.style.height = height + 'px';
    ...
    editPickUpRecommendList: function (geoLocation) {
      ...
      var recommendList = BX.create('DIV', {
        ...
        length =
          this.pickUpPointsJSON.length <
          this.options.pickUpMap.nearestPickUpsToShow
            ? this.pickUpPointsJSON.length
            : this.options.pickUpMap.nearestPickUpsToShow,
        i,
        pointsGeoQuery,
      ...
      for (i = 0; i < length; i++) {
        ...
        distance = ymaps.coordSystem.geo.getDistance(
        ...
        selectedDelivery.STORE_MAIN.splice(
          selectedDelivery.STORE_MAIN.indexOf(storeId),
          1
        );
    ...
    getDeliverySortedArray: function (objDelivery) {
      ...
      for (k in objDelivery) {
        if (objDelivery.hasOwnProperty(k)) arDelivery.push(objDelivery[k]);
      }
    ...
    editPropsItems: function (propsNode) {
      ...
      while ((group = groupIterator())) {
        if (group.getId() === '2' || group.getId() === '4') continue;
        propsIterator = group.getIterator();
        while ((property = propsIterator())) {
          if (
            this.deliveryLocationInfo.loc == property.getId() ||
    ...
    validateString: function (input, arProperty, fieldName) {
      ...
      if (arProperty.REQUIRED == 'Y' && value.length == 0)
        ...
        if (value.length > 0 && arProperty.IS_EMAIL == 'Y') {
          re =
            /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
          if (!re.test(value)) errors.push(BX.message('SOA_INVALID_EMAIL'));
        }
      }
    ...
    validateLocation: function (input, arProperty, fieldName) {
      ...
      if (
        ...
        (value.length == 0 || value == BX.message('SOA_NOT_SPECIFIED'))
      )
        errors.push(field + ' ' + BX.message('SOA_REQUIRED'));
    ...
    validateNumber: function (input, arProperty, fieldName) {
      ...
      if (value.length) {
        ...
        if (arProperty.STEP && parseFloat(arProperty.STEP) > 0) {
          ...
          if (del != parseInt(del))
            errors.push(
              field + ' ' + BX.message('SOA_NUM_STEP') + ' ' + arProperty.STEP
            );
        }
      }
    ...
    validateEnum: function (inputs, arProperty, fieldName) {
      ...
      for (i = 0; i < inputs.length; i++)
        if (inputs[i].checked || inputs[i].selected) values.push(i);
    ...
    validateSelect: function (inputs, arProperty, fieldName) {
      ...
      for (i = 0; i < inputs.length; i++)
        if (inputs[i].selected) values.push(i);
    ...
    validateFile: function (inputs, arProperty, fieldName) {
      ...
      if (
        ...
        files.length == 0 &&
        defaultValue == '' &&
        !arProperty.DEFAULT_VALUE.length
      )
        errors.push(field + ' ' + BX.message('SOA_REQUIRED'));
      else {
    ...
    editTotalBlock: function () {
      ...
      this.totalInfoBlockNode.appendChild(
        this.createTotalUnit(
          $('#js_you_order').text(),
          '<a href="/personal/cart/" class="bx-soa-editstep">' +
            $('#js_edit').text() +
            '</a>'
        )
      );
      ...
      this.totalInfoBlockNode.appendChild(
        this.createTotalUnit(BX.message('SOA_SUM_SUMMARY'), priceHtml)
      );
      ...
      if (this.options.showTaxList) {
        for (i = 0; i < total.TAX_LIST.length; i++) {
          ...
          this.totalInfoBlockNode.appendChild(
            this.createTotalUnit(total.TAX_LIST[i].NAME, valFormatted)
          );
        }
      }
      ...
      deliveryValue = deliveryError
        ? BX.message('SOA_NOT_CALCULATED')
        : total.DELIVERY_PRICE_FORMATED;

      if (parseFloat(total.DELIVERY_PRICE) >= 0 && this.result.DELIVERY.length)
        this.totalInfoBlockNode.appendChild(
          this.createTotalUnit(BX.message('SOA_SUM_DELIVERY'), deliveryValue, {
            error: deliveryError,
          })
        );
            ...
            children: [
              BX.create('A', {
                props: {
                  ...
                  className: 'btn btn-default btn-size-L btn-order-save',
                },
      ...
      return BX.create('DIV', {
        props: {
          className:
            ...
            (!!params.total ? ' bx-soa-cart-total-line-total' : ''),
        },
        children: [
          BX.create('SPAN', {
  ...
  (function ($) {
    ...
    $(function () {
      if (!document.getElementById('bx-soa-total')) {
        return;
      }
      ...
      $(document).scroll(function () {
        var $sideButton = $('#bx-soa-total .btn-order-save');
        var $bottomButton = $('#bx-soa-orderSave .btn');

        var l =
          $bottomButton.offset().top +
          $bottomButton.height() -
          $('#bx-soa-total .bx-soa-cart-total').offset().top;

        if (l < window.innerHeight) {
          $sideButton.css({ visibility: 'hidden' });
        } else {
          $sideButton.css({ visibility: 'visible' });
        }
      });
    });
  })(window.jQuery);
