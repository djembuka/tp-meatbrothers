BX.saleOrderAjax = {
            ...
            if (locPropId !== false) {
              BX.bindDebouncedChange(input, function (value) {
                ...
                if (
                  ...
                  ctx.getLocationByZip(
                    value,
                    function (locationId) {
                      ctx.properties[locPropId].control.setValueByLocationId(
                        locationId
                      );
                    },
                    function () {
  ...
  getLocationByZip: function (value, successCallback, notFoundCallback) {
    ...
    var loader;
    if (!(loader = BX.Sale.OrderAjaxComponent.startLoader())) return;
    ...
    BX.ajax({
      ...
      onsuccess: function (result) {
        ...
        } else notFoundCallback.call(ctx);
      },
...
(function ($) {
  ...
  $(function () {
    $('#consentModal button.btn-default:first').click(function () {
      $('#bx-soa-orderSave input:checkbox').prop('checked', true);
      $('#consentModal .modal-header .close').click();
      $('.bx-soa-button-block').addClass('i-available');
    });
    $('#consentModal button.btn-reset').click(function () {
      $('#bx-soa-orderSave input:checkbox').prop('checked', false);
      $('#consentModal .modal-header .close').click();
      $('.bx-soa-button-block').removeClass('i-available');
    });
    $('.bx-soa-button-block').click(function (e) {
      if ($('#bx-soa-orderSave input:checkbox').prop('checked') === false) {
        $('#consentModal').modal('show');
      }
    });

    $('#bx-soa-orderSave label a')
      .off('click')
      .click(function (e) {
        e.stopPropagation();
      });

    var json = $('#bx-soa-orderSave label').data('bx-user-consent');
    var sessid = $('#bx-soa-order-form #sessid').val();
